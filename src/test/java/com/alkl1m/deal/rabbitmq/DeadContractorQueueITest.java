...
public class DeadContractorQueueITest {
    ...
    void testDeadQueue_withValidMessage_willResendToDealContractorQueue() throws InterruptedException {
        ...
        rabbitTemplate.convertAndSend(MQConfiguration.DLX_EXCHANGE_MESSAGES, MQConfiguration.DEAL_CONTRACTOR_QUEUE, message, m -> {
        ...
        assertEquals(rabbitAdmin.getQueueInfo(MQConfiguration.DEAL_CONTRACTOR_QUEUE).getMessageCount(), 1);
        assertEquals(rabbitAdmin.getQueueInfo(MQConfiguration.DEAL_CONTRACTOR_DLQ).getMessageCount(), 0);
    }
}
